...
namespace App\Http\Controllers\Dashboard\Requestkamar\Permintaan;
...
class MainController extends Controller
{
    public function index()
    {
        ...
        return view('contents.dashboard.requestkamar.permintaan.main', $data);
    }
    public function datatablepermintaankamar()
    {
        $pembayaran = Requestpembayaran::where('status_verifikasi', 0)->orderby('created_at', 'DESC')->get();
        ...
        foreach ($pembayaran as $row) {
            ...
            <div class="d-flex align-items-center justify-content-center gap-1">
                <a href="' . route('request.permintaankamar.tambah', encrypt($row->no_request)) . '" class="btn btn-info fw-bold d-flex align-items-center justify-content-center" data-bs-toggle="tooltip" title="Lihat Request" style="width: 40px;">
                    <i class="fa fa-eye"></i>
                </a>
            </div>
            ';
    ...
    public function tambah($no_request)
    {
        $no_request = decrypt($no_request);

        $requestpembayaran = Requestpembayaran::where('no_request', $no_request)->first();
        $requestpembayarandetail = Requestpembayarandetail::where('no_request', $no_request)->get();

        $data = [
            'judul' => 'Permintaan kamar',
            'requestpembayaran' => $requestpembayaran,
            'requestpembayarandetail' => $requestpembayarandetail
        ];

        return view('contents.dashboard.requestkamar.permintaan.tambah', $data);
    }
}
