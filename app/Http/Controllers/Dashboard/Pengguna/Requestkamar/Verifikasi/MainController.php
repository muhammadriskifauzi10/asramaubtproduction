...
namespace App\Http\Controllers\Dashboard\Pengguna\Requestkamar\Verifikasi;
...
class MainController extends Controller
{
    public function index($penyewa_id)
    {
        ...
        return view('contents.dashboard_mahasiswa.requestkamar.verifikasi.main', $data);
    }
    public function datatableverifikasipermintaankamar()
    {
        $penyewa_id = request()->input('penyewa_id');

        $pembayaran = Requestpembayaran::where('penyewa_id', $penyewa_id)->where('status_verifikasi', 1)->orderby('created_at', 'DESC')->get();
